...
public class UsuarioSATest {
	...
	public void AnadirUsuarioTest() {
		usuario.AnadirUsuario(new UsuarioPOJO("jaime123", "Jaime Fernandez",
				109, "soyjaime", "Spain"));
	...
	public void AnadirAlarmaTest() {
		usuario.AnadirAlarma(
				new AlarmaPOJO("al16", 12, 23, true, "mytone1.mp3"), "jc");
		assertTrue("Debería encontrarla", alarmaDAO.getFromId("al16") != null);
		assertTrue("Debería añadir la alarma", alarmaUsuarioDAO
				.getAlarmasUsuario("jc").contains("al16"));
		usuario.AnadirAlarma((AlarmaPOJO) alarmaDAO.getFromId("al2"),
				"pablitos");
		assertTrue("Deberia haber anadido una alarma existente",
				alarmaUsuarioDAO.getAlarmasUsuario("pablitos").contains("al2"));
	}
	 ...
	 */
	/*
	 * @Test public void AnadirPreguntaTest() { ArrayList<String>
	 * respuestas1=new ArrayList<String>(); respuestas1.add("Madrid");
	 * respuestas1.add("Paris"); respuestas1.add("Roma");
	 * respuestas1.add("Londres"); preguntaDAO.save(new PreguntaPOJO("a1",
	 * "cual es la capital de españa?", "geografía", respuestas1, 1));
	 * 
	 * usuario. assertTrue("Deberia encontrarla",
	 * preguntaUsuarioDAO.getPreguntas("javigm").contains("a1")); }
	 */
	...
	public void EliminarAlarmaTest() {
		...
		assertTrue("No debería tener asignada esta alarma", !alarmaUsuarioDAO
				.getAlarmasUsuario("javigm").contains("al1"));
	}
